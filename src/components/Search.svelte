<script>
  ...
  const getToken = async () => {
    ...
    if (!$access_token || $expiration < current_time) {
  ...
  const getFlights = async (origin, price) => {
    ...
    let response = await fetch(
      `https://test.api.amadeus.com/v1/shopping/flight-destinations?origin=${origin}&maxPrice=${price}&departureDate=${formatDate(
        ...
        Math.floor(Math.abs(arrivalDate - departureDate) / 1000 / 3600 / 24) + 1
      }`,
      {
        method: "GET",
        headers: {
...
<div class="w-full md:h-20 sm:h-82 m-auto pt-1 bg-dark-gray text-dark-gray">

  <div class="md:max-w-screen-xl md:flex m-auto">

    <div class="md:flex md:w-full mx-5">

      <div class="md:flex-shrink-0 md:flex md:w-full md:space-x-2 md:space-y-0 sm:space-y-2 ">
        ...
        <button class="bg-red-600 hover:bg-red-500 active:bg-red-400 
                      focus:outline-none 
